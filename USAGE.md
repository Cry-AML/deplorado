...
本文档提供了 DepLorado Rust 版本的详细使用说明和实际示例。
...
# 2. 查看可用的数据类型
./target/release/deplorado list --data-type CRISPR

# 3. 下载 CRISPR 数据
./target/release/deplorado download --data-type CRISPR
```
...
```bash
# 列出所有发布版本
./target/release/deplorado list

# 列出特定数据类型的数据集
./target/release/deplorado list --data-type CRISPR

# 显示详细信息（包含文件数量、大小等）
./target/release/deplorado list --detailed

# 列出特定发布版本的文件
./target/release/deplorado list --release "DepMap Public 25Q3"
```
...
# 下载特定的数据集（所有相关文件）
./target/release/deplorado download --dataset Chronos_Combined

# 下载单个特定文件
./target/release/deplorado download --file "CRISPRGeneEffect.csv"
...
# 搜索所有内容（默认行为）
./target/release/deplorado search "gene dependency"

# 限制搜索结果数量
./target/release/deplorado search "cancer" --limit 10
```
...
# 4. 搜索特定基因
./target/release/deplorado search "TP53" --dataset --limit 5
```
...
```bash
# 1. 搜索细胞系
./target/release/deplorado search "MCF7" --cell-line

# 2. 查看数据集
./target/release/deplorado list --detailed

# 3. 下载基因表达数据（通常包含细胞系信息）
./target/release/deplorado download --dataset expression

# 4. 下载突变数据
./target/release/deplorado download --dataset mutations_damaging
```
...
```bash
# 药物敏感性研究
./target/release/deplorado download --dataset GDSC2_AUC

# 蛋白质表达数据
./target/release/deplorado download --dataset "Harmonized MS CCLE Gygi"

# 代谢组学数据
./target/release/deplorado download --dataset metabolomics
```
...
```toml
[dependencies]
deplorado = { path = "/path/to/deplorado" }
...
```rust
use deplorado::{CacheManager, Downloader};
...
```bash
# 高速网络（100+ Mbps）
./target/release/deplorado download --workers 8

# 普通网络（10-50 Mbps）
./target/release/deplorado download --workers 4  # 默认值
...
# 仅下载必要文件
./target/release/deplorado download --dataset CRISPR
```
...
# 降低并发数以减少网络压力
./target/release/deplorado download --workers 1
```
...
./target/release/deplorado download --output depmap_data/crispr --data-type CRISPR
```
...
```bash
# 记录下载的版本信息
./target/release/deplorado list > downloads_log.txt
./target/release/deplorado stats >> downloads_log.txt
```

这个使用指南涵盖了 DepLorado Rust 版本的所有主要功能和使用场景，帮助用户快速上手并高效使用工具进行癌症依赖性数据的研究和分析。
